...
SELECT Dept, cast(count(case when s.Gender = 'F' then 1 end) AS FLOAT)/ count(Gender) as Percentage FROM gpa_active_complete as gac  JOIN Students as s ON gac.StudentId = s.StudentId JOIN Degrees as d ON d.DegreeId = gac.DegreeId GROUP BY Dept HAVING Dept = %1%;
SELECT 0;
SELECT 0;
SELECT 0;
SELECT 0;
